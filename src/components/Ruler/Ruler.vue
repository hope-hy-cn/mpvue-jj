<template>
    <div>
      ruler...
      <scroll-view scroll-x="true" @scroll="bindscroll">
      <canvas
        class="canvas-ruler"
      ...
      <div>{{options.canvasWidth}} {{options.id}}</div>
        ...
        // ['options'],
          {
            options:{
              ...
              scrollLeft: 0,
              scrollLeftNow:0,
              scaleNum: 0,  //每两个数字之间的刻度数
              bgColor: "#F6F6F6",
              nowVal: 0,
              maxVal: 100,
              minVal: 0,
              accuracy: "", //精度，刻度最小值
              autoScale: true, //是否自动调整刻度
              scaleStep: "",    //刻度数字跨度
              scaleWidth: "",   //每个刻度所占的宽度
              hasMidScale: true,   //是否有中间刻度
              onlySelectScale: false,  //只选中刻度上标识出数字的值
              scaleColor: "",
              scaleLineWidth: "",
              scaleTextColor: "",
              scaleTextSize: "",
              selectScaleColor: "",
              selectScaleTextColor: "",
              selectScaleTextSize: "",
              strong: false,  //选中项是否变大突出
           }
        },
      onReady(e) {
        // 使用 wx.createContext 获取绘图上下文 context
        const context = wx.createCanvasContext('dd')
        this.drawRuler()
      },
      methods:{
